...
from .utils_models import GlobalParameters
from .utils_models import Config
from .utils_models import Params
from .utils_models import OpenApiRequest
"""
 * @remarks
...
class Client:
    ...
    def do_rpcrequest(
        ...
        _runtime = {
            'key': str(runtime.key or self._key),
            'cert': str(runtime.cert or self._cert),
            'ca': str(runtime.ca or self._ca),
            ...
            'httpProxy': str(runtime.http_proxy or self._http_proxy),
            'httpsProxy': str(runtime.https_proxy or self._https_proxy),
            'noProxy': str(runtime.no_proxy or self._no_proxy),
            'socks5Proxy': str(runtime.socks_5proxy or self._socks_5proxy),
            'socks5NetWork': str(runtime.socks_5net_work or self._socks_5net_work),
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                ...
                _request.protocol = str(self._protocol or protocol)
                ...
                if not DaraCore.is_null(request.body):
                    m = request.body
                    tmp = Utils.query(m)
                    _request.body = DaraCore.to_readable_stream(DaraForm.to_form_string(tmp))
                ...
                if body_type == 'binary':
                    ...
                    res_str = DaraStream.read_as_string(_response.body)
                    return {
                        'body': res_str,
                        'headers': _response.headers,
                        'statusCode': _response.status_code
    ...
    async def do_rpcrequest_async(
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                ...
                headers = await self.get_rpc_headers_async()
                if DaraCore.is_null(headers):
                    # endpoint is setted in product client
                    _request.headers = DaraCore.merge({
                        'host': self._endpoint,
                ...
                _last_response = _response
                if (_response.status_code >= 400) and (_response.status_code < 600):
                    _res = DaraStream.read_as_json_async(_response.body)
                    err = _res
                    request_id = err.get("RequestId") or err.get("requestId")
                    code = err.get("Code") or err.get("code")
                    if (f'{code}' == 'Throttling') or (f'{code}' == 'Throttling.User') or (f'{code}' == 'Throttling.Api'):
                        ...
                        raise main_exceptions.ClientException(
                            ...
                            message = f'code: {_response.status_code}, {err.get("Message") or err.get("message")} request id: {request_id}',
                            description = f'{err.get("Description") or err.get("description")}',
                            access_denied_detail = await self.get_access_denied_detail_async(err),
                            request_id = f'{request_id}'
                        )
                    else:
                        raise main_exceptions.ServerException(
                ...
                if body_type == 'binary':
                    ...
                    byt = DaraStream.read_as_bytes_async(_response.body)
                    return {
                        'body': byt,
                        'headers': _response.headers,
                        'statusCode': _response.status_code
                    ...
                    res_str = DaraStream.read_as_string_async(_response.body)
                    return {
                        'body': res_str,
                        'headers': _response.headers,
                        'statusCode': _response.status_code
                    ...
                    obj = DaraStream.read_as_json_async(_response.body)
                    res = obj
                    return {
                        'body': res,
                        'headers': _response.headers,
                    ...
                    arr = DaraStream.read_as_json_async(_response.body)
                    return {
                        'body': arr,
                        'headers': _response.headers,
                        'statusCode': _response.status_code
    ...
    def do_roarequest(
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                ...
                if not DaraCore.is_null(request.body):
                    _request.body = DaraCore.to_readable_stream(json.dumps(request.body))
                ...
                if (_response.status_code >= 400) and (_response.status_code < 600):
                    _res = DaraStream.read_as_json(_response.body)
                    err = _res
                    request_id = str(err.get("RequestId") or err.get("requestId"))
                    request_id = str(request_id or err.get("requestid"))
                    code = str(err.get("Code") or err.get("code"))
                    if (f'{code}' == 'Throttling') or (f'{code}' == 'Throttling.User') or (f'{code}' == 'Throttling.Api'):
                        raise main_exceptions.ThrottlingException(
                            status_code = _response.status_code,
                            code = f'{code}',
    ...
    def do_roarequest_with_form(
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                ...
                if not DaraCore.is_null(request.body):
                    m = request.body
                    _request.body = DaraCore.to_readable_stream(Utils.to_form(m))
                    _request.headers["content-type"] = 'application/x-www-form-urlencoded'
                _request.query = DaraCore.merge({}, global_queries, extends_queries)
                if not DaraCore.is_null(request.query):
                    _request.query = DaraCore.merge({}, _request.query, request.query)
    ...
    def do_request(
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                _request = DaraRequest()
                _request.protocol = str(self._protocol or params.protocol)
                ...
                signature_algorithm = str(self._signature_algorithm or 'ACS3-HMAC-SHA256')
                ...
                if not DaraCore.is_null(request.stream):
                    ...
                    _request.body = DaraCore.to_readable_stream(tmp)
    ...
    async def do_request_async(
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                ...
                hashed_request_payload = Utils.hash(DaraBytes.from_('', 'utf-8'), signature_algorithm)
                if not DaraCore.is_null(request.stream):
                    tmp = DaraStream.read_as_bytes_async(request.stream)
                    hashed_request_payload = Utils.hash(tmp, signature_algorithm)
                    _request.body = DaraCore.to_readable_stream(tmp)
                    _request.headers["content-type"] = 'application/octet-stream'
                else:
                ...
                if (_response.status_code >= 400) and (_response.status_code < 600):
                    err = {}
                    if not DaraCore.is_null(_response.headers.get("content-type")) and _response.headers.get("content-type") == 'text/xml;charset=utf-8':
                        _str = DaraStream.read_as_string_async(_response.body)
                        resp_map = DaraXML.parse_xml(_str, None)
                        err = resp_map.get("Error")
                    else:
                        _res = DaraStream.read_as_json_async(_response.body)
                ...
                if params.body_type == 'binary':
                    ...
                    anything = DaraStream.read_as_string_async(_response.body)
                    return {
                        'body': anything,
                        'headers': _response.headers,
                        'statusCode': _response.status_code
        ...
        while DaraCore.should_retry(_runtime.get('retryOptions'), _context):
            ...
            try:
                ...
                request_context = spi_models.InterceptorContextRequest(
                    ...
                    protocol = str(self._protocol or params.protocol),
                    method = str(self._method or params.method),
                ...
                configuration_context = spi_models.InterceptorContextConfiguration(
                    ...
                    endpoint = str(request.endpoint_override or self._endpoint),
